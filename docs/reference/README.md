...
| `telemetry-sensors.md` | How a value is matched to one of the model's telemetry sensors, how long that choice lasts, why a sensor the radio has never sent is not taken for a reading of zero, and the three different events that make the match happen again. | written | written |
